...
class NodeEposNow {
    ...
    _.forEach(api, (v) => {
      ...
      _.forEach(v.endpoints, (endpoint) => {
        ...
        _.forEach(endpoint.availables, (method) => {
          if (method === "update") {
            this[v.version][functionName][method] = (id, payload) => {
            ...
            this[v.version][functionName][method] = (params) => {
              return this[`_${method}FunctionBuilder`](
                `${path}/${endpoint.name}`,
                params
              );
  ...
  _updateFunctionBuilder(path, id, payload) {
    return new Promise(async (resolve, reject) => {
      try {
        let url = `${constants.BASE_URL}/${path}/${id}`;
        if (!payload) reject(new Error("'Request Body' must be provided!"));
        const res = await superagent
          .put(url)
          .set("Authorization", `Basic ${this.accessToken}`)
